...
import { useHouseTopWin24, useHouseTopLoss24, useHouseGlobalStatistics } from '../../api/hooks/info'

const FooterHouse = () => {
  const [isOpen, setOpen] = useState(true)

  const { list: winList, success: winSuccess, loading: winLoading } = useHouseTopWin24()
  const { list: lossList, success: lossSuccess, loading: lossLoading } = useHouseTopLoss24()
  const { list: globalList, success: globalSuccess, loading: globalLoading } = useHouseGlobalStatistics()
  ...
  return (
    <footer className="flex-shrink-0 w-full">
      <div className="flex justify-between px-5 py-2 bg-cE3122D rounded-t-2xl ">
        ...
        <div className="flex items-center">
          <svg
            className="mr-2 cursor-pointer group hidden"
            width="16"
            height="16"
            viewBox="0 0 16 16"
            fill="none"
