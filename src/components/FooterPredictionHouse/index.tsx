...
import {
  usePredictionHouseTopWin24,
  usePredictionHouseTopLoss24,
  usePredictionHouseGlobalStatistics,
} from '../../api/hooks/info'
...
const Footer = () => {
  ...
  const { list: winList, success: winSuccess, loading: winLoading } = usePredictionHouseTopWin24()
  const { list: lossList, success: lossSuccess, loading: lossLoading } = usePredictionHouseTopLoss24()
  const { list: globalList, success: globalSuccess, loading: globalLoading } = usePredictionHouseGlobalStatistics()
    ...
    <footer className="flex-shrink-0 w-full">
      <div className="flex justify-between px-5 py-2 bg-cE3122D rounded-t-2xl ">
        <div className="flex items-center">
          ...
          <span className="ml-2 font-sans font-bold text-white cursor-pointer text-10px" onClick={() => setTabIndex(0)}>
            STATS
          </span>
          <span className="ml-2 font-sans font-bold text-white cursor-pointer text-10px" onClick={() => setTabIndex(1)}>
            CHART
          </span>
        </div>
        <div className="flex items-center">
      ...
      <div className={`overflow-x-scroll lg:overflow-hidden ${isOpen ? 'h-64' : 'h-0'}`}>
        {tabIndex === 0 && (
          <div className="px-1.5 py-2 flex h-full">
            <div className="flex-none overflow-x-scroll min-w-[75%] sm:min-w-[45%] lg:min-w-[32.8%] h-full px-5 py-3 mx-1 border rounded-md bg-c202C3A border-cF0EFEE">
                  ...
                  <tbody>
                    {winList.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td>{item.total_bet.toFixed(3)} BOLD</td>
                        <td className={item.total_profit >= 0 ? 'text-green-600' : 'text-red-600'}>
                          {item.total_profit.toFixed(3)} BOLD
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                <table className="w-full font-sans text-xs leading-5 text-center text-cF0EFEE">
                  ...
                  <tbody>
                    {globalList.map((item, index) => {
                      ...
                      const totalBets = parseFloat(data.totalbulls) + parseFloat(data.totalbears)
                      const bullPercent = totalBets === 0 ? 0 : (parseFloat(data.totalbulls) * 100) / totalBets
                      const bearPercent = totalBets === 0 ? 0 : 100 - bullPercent
                      return (
                        <tr key={index}>
                          <td className="font-medium">{type}</td>
                          <td>{bullPercent.toFixed(2)}%</td>
                          <td>{bearPercent.toFixed(2)}%</td>
                        </tr>
                      )
                    })}
                  </tbody>
