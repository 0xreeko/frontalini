...
import { usePredictionTopWin24, usePredictionTopLoss24, usePredictionGlobalStatistics } from '../../api/hooks/info'

const Footer = () => {
  const [isOpen, setOpen] = useState(true)
  const [tabIndex, setTabIndex] = useState(1)

  const { list: winList, success: winSuccess, loading: winLoading } = usePredictionTopWin24()
  const { list: lossList, success: lossSuccess, loading: lossLoading } = usePredictionTopLoss24()
  const { list: globalList, success: globalSuccess, loading: globalLoading } = usePredictionGlobalStatistics()
