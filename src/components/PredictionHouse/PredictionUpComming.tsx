...
const PredictionUpComing: React.FC<Props> = ({ roundId, timeRemain }) => {
  ...
  const { start, stop } = useInterval(() => (time > 1 ? setTime(time - 1) : stop()), 1000, {
    autoStart: false,
    immediate: false,
    onFinish: () => {},
  })
...
export default PredictionUpComing
