...
import { COINTOSS_ADDRESS } from '../constants'
  ...
  const betTails = useCallback(
    async (epoch: number, value: number) => {
      ...
      const estimatedGas = await contract.estimateGas
        .betTails(BigNumber.from(epoch), { value: utils.parseEther(value.toString()), gasPrice: utils.parseUnits(gasPrice, 'gwei')})

      const tx = await contract
        .betTails(BigNumber.from(epoch), { value: utils.parseEther(value.toString()), gasPrice: utils.parseUnits(gasPrice, 'gwei'), gasLimit: calculateGasMargin(estimatedGas)})
...
export function useBetHeads(): [(epoch: number, value: number) => Promise<void>, boolean, boolean] {
  ...
  const betHeads = useCallback(
    async (epoch: number, value: number) => {
      ...
      const estimatedGas = await contract.estimateGas
        .betHeads(BigNumber.from(epoch), { value: utils.parseEther(value.toString()), gasPrice: utils.parseUnits(gasPrice, 'gwei')})
      const tx = await contract
        .betHeads(BigNumber.from(epoch), { value: utils.parseEther(value.toString()), gasPrice: utils.parseUnits(gasPrice, 'gwei'), gasLimit: calculateGasMargin(estimatedGas)})
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: 'BetHeads ',
          })
  ...
  const getUserBetHistory = useCallback(
    async (roundId: number, account: string) => {
      ...
      if (UserRound.length > 0) {
        ...
        const RoundUserHistory = await contract.ledger(userRoundEpoch, account)
        const amount = parseFloat(utils.formatEther(RoundUserHistory.amount))
        if (RoundHistory.tossResult.toString() === RoundUserHistory.position.toString()) {
          ...
          if (RoundHistory.tossResult.toString() === '1') {
            winAmount = (amount / HeadAmount) * TailAmount * 0.97 + amount
          } else if (RoundHistory.tossResult.toString() === '0') {
            winAmount = (amount / TailAmount) * HeadAmount * 0.97 + amount
          }
          return {
            status: 1,
            amount,
  ...
  const getLastBetHistory = useCallback(
    async (roundId: number, account: string) => {
      ...
      if (lastUserRound) {
        ...
        const RoundHistory = await contract.rounds(lastUserRound)
        const lastRoundUserHistory = await contract.ledger(userLastRoundEpoch, account)
        const amount = parseFloat(utils.formatEther(lastRoundUserHistory.amount))
        if (lastRoundHistory.tossResult.toString() === lastRoundUserHistory.position.toString()) {
          ...
          return { status: 1, amount, winAmount }
        } else return { status: 0, amount }
...
export function useRounds(): TossType[] {
  ...
  let result = round
    .map((v) => {
      ...
      const closePrice = v.result?.closeOracleId ? parseFloat(utils.formatUnits(v.result?.closeOracleId, 8)) : 0
      ...
      return [
        {
          ...
          status,
          winner: tossResult === '1',
          Headpayout: HeadAmount ? totalAmount / HeadAmount : 0,
          Tailpayout: TailAmount ? totalAmount / TailAmount : 0,
          winnerList,
          oracleCalled,
        },
      ]
  ...
  if (round && result.length > 0) {
    upComing = [
      ...
      },
      {
        ...
        Tailpayout: 2.3465157004904555,
        closePrice: 463.34,
        closeTimestamp: result[result.length - 1].closeTimestamp + 190,
        epoch: result[result.length - 1].epoch + 2,
        lockPrice: 463.1136569,
        lockTimestamp: result[result.length - 1].lockTimestamp + 190,
        status: 2,
        totalAmount: 53.03088226502455,
        winner: false,
      },
