...
import ABI from '../constants/abis/CWJPrediction.json'
...
export function useBetTails(): [(epoch: number, value: number) => Promise<void>, boolean, boolean] {
  const contract = useContract(PREDICTION_ADDRESS, ABI)
  ...
  const betTails = useCallback(
    async (epoch: number, value: number) => {
      ...
      const estimatedGas = await contract.estimateGas
        .betBear(BigNumber.from(epoch), { value: utils.parseEther(value.toString()), gasPrice: utils.parseUnits(gasPrice, 'gwei') })
      const tx = await contract
        .betBear(BigNumber.from(epoch), { value: utils.parseEther(value.toString()), gasPrice: utils.parseUnits(gasPrice, 'gwei'), gasLimit: calculateGasMargin(estimatedGas)})
...
export function useBetHeads(): [(epoch: number, value: number) => Promise<void>, boolean, boolean] {
  ...
  const betHeads = useCallback(
    async (epoch: number, value: number) => {
      ...
      const estimatedGas = await contract.estimateGas
      .betBull(BigNumber.from(epoch), { value: utils.parseEther(value.toString()), gasPrice: utils.parseUnits(gasPrice, 'gwei') })
      const tx = await contract
        .betBull(BigNumber.from(epoch), { value: utils.parseEther(value.toString()), gasPrice: utils.parseUnits(gasPrice, 'gwei'), gasLimit: calculateGasMargin(estimatedGas)})
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: 'betBull ',
          })
...
export function useGetOraclePrice(): number {
  ...
  return price ? parseFloat(price.toString()) / 10 ** 8 : 0
}
      ...
      if (UserRound.length > 0) {
        ...
        if (
          ...
          if (RoundHistory.closePrice >= RoundHistory.lockPrice && RoundUserHistory.position === 0) {
            winAmount = (amount / BullAmount) * BearAmount * 0.97 + amount
          } else if (RoundHistory.closePrice < RoundHistory.lockPrice && RoundUserHistory.position === 1) {
            winAmount = (amount / BearAmount) * BullAmount * 0.97 + amount
          }
          return {
            status: 1,
            amount,
      ...
      return [
        {
          ...
          Headpayout: BullAmount ? totalAmount / BullAmount : 0,
          Tailpayout: BearAmount ? totalAmount / BearAmount : 0,
          winnerList,
          oracleCalled,
        },
      ]
